...
class NEMModel(nn.Module):
    def __init__(self, d_h=256, d_a=256):
        ...
        self.to_act = nn.Sequential(
            nn.LayerNorm([d_a+d_h], elementwise_affine=False),
        ...
        self.norm = nn.LayerNorm([d_h], elementwise_affine=False),
    ...
    def inference(self, x, Y, dropout=False):
        ...
        for i in range(self.state['n_layers']):
            ...
            y = torch.matmul(w, y)  / math.sqrt(w.shape[1])
    ...
    def update(self, x, Y, dropout=False):
        ...
        for i in range(self.state['n_layers']):
            ...
            m_next = torch.matmul(self.state['l'+str(j)].t(), self.state['h'+str(j)]) / math.sqrt(self.state['h'+str(j)].shape[-1])
